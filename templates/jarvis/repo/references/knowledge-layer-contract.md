...
# Jarvis / cross-cutting / repo-local knowledge 分层合同
...
| repo-local skill/reference | 在一个 repo 中去哪里、怎样实现和验证 | architecture/layer owner、concrete source role、repo command、test home | Jarvis 级 workflow、跨 repo 产品矩阵 |
...
当 Jarvis route 命中 execution repo：
...
Jarvis knowledge 与当前 route scope 所需的 repo-local knowledge 交付后，必须把 pending handoff 替换为实际入口并重新验证路由。
...
6. 是Jarvis无关的通用推理方法？留在 runtime method，不复制进 Jarvis repo。

若同时包含跨模块 contract 和仓库实现，拆成两层：jarvis 保存 contract 与 pointer，repo-local 保存实现与 proof。
...
- jarvis capability owner 与代码 semantic owner 可以不同；必须写清 handoff，不能互相覆盖。
...
- Jarvis无关的 agent reasoning gate → upstream runtime method。
