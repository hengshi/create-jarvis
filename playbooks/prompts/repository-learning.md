...
`History range` 使用 `BUILD-CONTEXT.md` 中客户选择并解析后的范围：最近一年、两年、全部可达历史或自定义日期/ref。每个 repo 的状态只使用 `pending`、`in-progress`、`completed`、`blocked`。表后只为当前 episode 记录恢复所需的信息：case 目录、START pointer、历史 outcome pointer、当前 replay 状态和下一动作。
...
9. **最小写回。** 确有可复用缺口时，使用 runtime 已安装的 `skill-creator` 修改唯一正确的 repo-local primary home。主 skill 保持短；细节优先进入 focused reference 或确定性脚本。
...
遇到缺少授权、无可验证 outcome、隔离运行不可用或 repo 是 read-only 时，把该 repo 标为 `blocked`，写清已搜索范围和恢复动作，然后继续其他 repo。不要因为一个 repo 阻塞而丢失整个队列的进度。

只有 requested range 内所有可达 commits 都有 code-read coverage、所有选中 episode 都已闭合或明确 blocked，并且累计 guidance 已在当前 revision 收口验证，repo 才能标为 `completed`。任务结束时只总结各 repo 的范围与覆盖状态、保留的 skill delta、blocker 和同一启动命令的恢复方式；不向客户解释内部 eval 术语。
