...
目标：从 `templates/company-jarvis/` 确定性实例化客户 company Jarvis repo 的结构骨架——root、canonical company entry、完整 17 baseline references、cross-cutting、modules、sources README、tools inventory、root runtime contracts。

Phase 7 的职责是建立容器和路由骨架，并把 Phase 6 已确认的客户事实写入 module overview、source route 和 first routing。它不负责自由生成 workflow/source-helper skills——那属于 Phase 9。
...
- `skills/<company>-jarvis/SKILL.md` 是统一入口；
- workflow / source / helper 能力是 `skills/<skill>/SKILL.md`（由 Phase 9 创建）；
...
- workflow skills（`skills/<workflow>/SKILL.md`）
...
1. 运行确定性母版实例化器 base 子命令：
   ...
   ```
   这会将 `templates/company-jarvis/repo/` 完整渲染到 `paths.jarvis_target_home`，包括 root files、17 references、canonical entry、cross-cutting、tools inventory、evals skeleton。
...
Phase 7 `completed` 的含义：容器骨架已就位，Phase 9 可以开始按 generation plan 实例化完整的 workflow/source-helper skill packages。
...
- Phase 7 自由生成了 workflow skill 或 source-helper skill 文件（这些是 Phase 9 的职责）。
- company entry skill 没有落在 `skills/<company>-jarvis/SKILL.md`。
