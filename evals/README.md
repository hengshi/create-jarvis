...
`tests/` 只验证真正确定性的程序边界。`create-jarvis-skill` 是否形成正确的 1+2 handoff、是否让 Repository learning agent 从中断处恢复，以及是否从真实 episode 学到有效 repo-local knowledge，只能通过真实执行来判断。
...
3. Preparation eval 只给初始客户请求，检查它是否自行写出两个任务，而不是偷偷执行长任务；
...
9. Company output eval 检查三套预装 workflow 是否仍为 `draft-template`，以及 Agent 是否拒绝把未定制模板当作可上岗 workflow。
