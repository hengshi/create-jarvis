# Part 2: Jarvis construction

你的唯一目标，是从 Part 1 已交付的 Jarvis 骨架继续，依据 `BUILD-CONTEXT.md` 中已授权的客户证据构建客户自己的 Jarvis repo，并在验证后把它发布到客户确认的 GitHub 或 GitLab。

你不是在“把模板文件填满”，也不是要机械复制另一套 Jarvis 运行数月后才形成的文件数量。你的交付物应是一个对**本次声明授权范围与记录深度 coverage-complete、且所有结论 evidence-backed** 的 Jarvis 语义与路由中枢：新的 Agent 能据此理解这套 Jarvis 的用途与边界、有哪些能力、事实去哪里找、任务先查什么、何时进入哪个 repo，以及哪些结论仍未被证明。
...
1. 这套 Jarvis 的 confirmed identity、用途和运行边界是什么，相关产品名或历史名称有哪些冲突；
...
8. 一条新知识应留在 jarvis module、cross-cutting、source route、repo-local skill 还是当前任务中。
9. 所有使用这套 Jarvis 的 Agent 应遵守什么执行宪法，Runtime Foundation 是否已经安装并验证。

当前客户证据足以形成的 module knowledge、source route、跨模块关系和Jarvis 级决策，应在本轮完成，不能以“以后会成熟”为由跳过。成熟客户 workflow、额外 references 和专用 tools 则必须按各自证据形成；不能为了模仿另一个成熟 Jarvis 而按数量制造。只有需要判断跨 Part 生命周期或向客户解释整体成长路径时，才读取 `playbooks/customer-jarvis-growth-loop.md` 的相关章节。

## 写入边界

- 主要写入目标是任务文件指定的 Jarvis target、`BUILD-CONTEXT.md` 明确确认的 customer-owned GitHub/GitLab remote，以及客户明确批准并记录在 context/card 中的 Host runtime foundation 安装目标。
- 任务目录中只更新 `work/jarvis-construction.md` 和确有并行扫描需要时的 task-local evidence packet；delegated worker 把 verified card pointer 报给 Coordinator，由 Coordinator 单独更新 `CONSTRUCTION-JOURNAL.md`。不要建立 construction 状态机。
...
- 不运行 Repository learning 的 history eval loop。Jarvis construction 可以定向读取 issue、MR/PR、commit diff 和测试来证明Jarvis 级事实，但不遍历历史来训练 repo-local skill。
- 不创建 `jarvis.toml`、`bootstrap-state.json` 或 `bootstrap-result.json`。
- 当前是 Host Agent 的 construction context，不要求 jarvis-box 已安装，也不执行 `jarvis-box version/status/agent current`。生成的 jarvis 指针必须允许 construction/onboarding 在普通授权 checkout 中继续；正式 runtime 诊断只在后续 managed-production context 中触发。
- jarvis-box 的 injected execution contract、control plane 和 operator runbook 不复制进 Jarvis repo。Jarvis `runtime-governance.md` 是客户跨 runtime 宪法，必须在本任务中从 scaffold 建设为客户特定、可验证的规则。
- base 中的 issue post-check、bugfix 和 feature-delivery workflow 是待客户化的 starter。没有证据时保持明确未验证，不得把通用母版冒充客户 workflow。
...
- remote URL 必须对应 canonical `<jarvis-slug>-jarvis`，不能因为当前账号有权限就改发到个人 namespace；
...
1. 当前 `work/jarvis-construction.md`；
2. `BUILD-CONTEXT.md`；
3. `CONSTRUCTION-JOURNAL.md`；
4. jarvis target 当前内容和 Git 状态；
...
1. **用途锚点**：产品文档、UI、API contract、验收材料或行为测试证明这项客户能力真实存在；
...
统计、label 或 commit message 只用于发现候选。任何要写入 Jarvis 的 durable 结论，都要继续读取必要的正文、讨论、代码变化、测试或最终结果来证明；不能把一批 message 的语义分类直接当成知识。

### 如何并行

当 artifact 数量足够大且当前 runtime 支持 delegation 时，按互不重叠的 source、repo 或 module batch 并发扫描。每个 worker 只交付带 pointer 的 evidence packet，不直接同时修改同一个 jarvis target。主 Agent 负责 identity reconciliation、taxonomy、冲突判断和最终写入。
...
- 区分 jarvis identity、product identity、brand identity 和 source 中出现的历史名称。
- source 检测到的名称不能覆盖客户确认的Jarvis身份；冲突写 `needs-owner-confirmation`，并保留双方 pointer。
...
先核验 `work/jarvis-repo-initialization.md` 记录的本地与远端 commit，确认当前 target 基于 Part 1 的真实交付。若 scaffold 不存在、未通过 verifier 或 remote ref 不可解析，返回 Part 1 修复，不在 Part 2 重新初始化另一个 Jarvis repo。

Part 1 的 `base` 只负责安全、一致地生成：

- Jarvis entry 与通用目录；
- jarvis knowledge-layer / routing references；
- 三个 `unverified` workflow；
- 空的 tools/evals/cross-cutting 容器。

它没有证明任何客户语义。Part 2 可以用 `instantiate_jarvis.py module/source` 增加证据支持的容器，但不得用重复 base 渲染覆盖已经编辑的客户内容。

### 4. 逐 capability 做 evidence closure

对每个 `include` 的 capability，使用 `instantiate_jarvis.py module` 生成容器，然后完成最小 module contract：

- **业务定位**：用客户语言写目的、用户/角色、能力边界和明确 non-scope；
- **用途锚点**：指向实际 docs/UI/API/test evidence；
...
对每个已授权 source 使用 `instantiate_jarvis.py source` 建立一份 route contract，记录：
...
一个 repo 可以承担多个 surface；capability authority 也可能不是 repo。Jarvis construction 与 Repository learning 可以并行，因此 repo-local skill 尚未生成不是 Jarvis construction 的失败。记录当前真实入口；不存在时明确写 `pending Repository learning`，后续 Reconciliation Gate 再替换。
...
### 8. 建设并验证 runtime governance 与 Runtime Foundation
...
只从客户明确提供或随后明确授权的 Runtime Environment、操作规则和运行证据中发现并决定。若 `BUILD-CONTEXT.md` 尚无 runtime discovery/write scope，先说明这些信息将用于建立可持续同步、Agent discovery 与故障恢复，再取得授权；不得枚举 home、shell profile、Agent config 或旧 runtime residue 来猜：

- 每个 Runtime Environment 的 Agent HOME 和原生 skill discovery roots；
- Runtime Foundation 的 canonical cache、stable bin、state 与 log；
- approved Jarvis remote/ref 与 bootstrap authentication boundary；
- task-start quick sync、periodic full sync 和真实 Agent discovery probe；
...
- native scheduler 如何直接调用内部 Runtime Job；
- Docker host scheduler 如何经 jarvis-box release `runtime-job` helper 调用同一个内部任务；
- Host construction 与正式 Docker Runtime Environment 如何共享 Jarvis 宪法而不共享 Host HOME 或凭据。

不要复制 HENGSHI 的 `~/.hengshi`、`pullall` 或其他名字到另一套 Jarvis。客户已经存在等价机制时，记录真实路径、owner、调用方式和 evidence；没有机制但当前宪法要求时，在 Jarvis repo 中创建最小 bootstrap/sync/doctor 与 Scheduler Adapter，安装到客户决定的稳定位置并实际运行验证。

内部 Runtime Job 对 Docker 无感。禁止在 sync、maintenance 或 self-improve 中写 `docker compose exec/run`。Docker 差异只能出现在 Scheduler Adapter：宿主 scheduler 调用 jarvis-box release helper，由 helper 进入正式容器并执行同一个 inner command。Runtime Job 将自己的 state/log 写入持久 Agent HOME，host scheduler 只记录进入容器失败等外层错误。

Part 2 可以先完成 native/Host 环境验证。正式 Docker 环境在 Part 4 使用持久 Agent HOME 进行从零 bootstrap、sync、doctor 和真实 Agent discovery 验证。不得以 Jarvis 目录 mount、`JARVIS_HOME`、Jarvis context 或 deployment lock 补偿 Runtime Foundation 缺失。
...
- starter workflows 的真实客户化/验证状态和 onboarding next step。

初始 entry 在 workflow 尚未激活时，仍应能把知识查询、问题 artifact 和变更线索路由到正确 module/source/first proof；它不能假装已拥有客户 bugfix/feature 闭环。

不要为了模仿成熟 Jarvis 而自动生成额外 workflow、reference 或 tool。只有当前客户证据证明它是跨任务、可复用且 primary home 确实在 jarvis 层时才创建。
...
对每个 probe，从 Jarvis entry 开始，检查能否得到：
...
把期望、实际 route、差异和修正记录在 work card/evidence；适合长期回归且不暴露敏感内容的 case 再写入 jarvis `evals/`。修正 routing 后重跑失败 probe。不要用关键词存在、文件数量或 Python `assertIn` 代替这项行为验证。

### 11. 验证语义与待发布内容

运行本 method pack 的 `scripts/verify_jarvis_output.py` 检查它能确定判断的结构、安全、secret、模板 token 和写入边界。它通过只表示结构边界通过，不表示 jarvis 语义正确。
...
- starter workflow 没有虚构客户事实或验证结果；
- 客户 repo 未被修改；
- Jarvis repo 中没有 raw source dump、凭据或 construction state。

发布前再检查本地 Git 状态和准备提交的完整 diff，确认只包含 Jarvis 目标文件，没有临时 evidence packet、客户 raw source dump、credential、未渲染 token 或无关 worktree 变化。结构 verifier 通过不能替代这项 diff 与语义审查。
...
1. 在 jarvis target 中建立正常、可追溯的 Git history；仅提交已验证的 Jarvis 文件。
...
把 provider、remote URL、visibility、default branch、publication mode、本地/远端 branch、commit SHA、PR/MR URL 与状态、最后验证命令及结果写入 `work/jarvis-construction.md`。不得记录 token 或其他凭据。

## 完成标准

满足以下条件时，Jarvis construction 可以在当前授权边界内标记完成：

- jarvis/product identity 已确认，或冲突被明确标注且不妨碍已交付路由；
- 所有已授权 source/repo 都已读取并在 coverage 表中有状态，不可访问项有恢复条件；
- 每个 included software module 都完成用途锚点 + 实现锚点 + first proof/verification contract；
...
- starter workflow 的客户化和验证状态与证据一致；
...
- 只有当 blocker 使最小 Jarvis entry 无法形成时，才阻塞整体任务。

结束时只向客户说明：Jarvis 当前可用到什么边界、runtime governance 哪些行为已验证或仍缺 foundation、正式 remote URL（或 publication blocker）、需要 review 的 PR/MR URL（如有），以及 Reconciliation Gate 的下一步。不得代替独立 Repository learning card 声称完成，也不得声称数字员工已经正式上岗。
