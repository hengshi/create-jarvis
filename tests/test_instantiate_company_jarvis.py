#!/usr/bin/env python3
"""Behavior tests for deterministic company-Jarvis template rendering."""
...
class CompanyJarvisInstantiationTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.fixture = json.loads(INPUT_FIXTURE.read_text(encoding="utf-8"))

    def setUp(self) -> None:
        self.temp = tempfile.TemporaryDirectory(prefix="company-jarvis-")
    ...
    def test_base_creates_company_owned_shape_without_runtime_state(self) -> None:
    ...
    def test_repeat_preserves_company_owned_edits(self) -> None:
    ...
    def test_late_company_discovery_updates_navigation(self) -> None:
    ...
    def test_extension_packages_use_company_namespace(self) -> None:
        self.assertEqual(self.run_base().returncode, 0)
        for kind, name in (
    ...
    def test_unsafe_slug_fails_before_writing(self) -> None:
        def wrong_entry(value: dict) -> None:
            value["company"]["slug"] = "../another-company"
    ...
    def test_render_preflight_fails_without_partial_output(self) -> None:
        spec = importlib.util.spec_from_file_location("instantiate_company_jarvis", INSTANTIATOR)
