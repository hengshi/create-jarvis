...
VERIFIER_SCRIPT = REPO_ROOT / "scripts" / "verify_jarvis_output.py"
INSTANTIATOR = REPO_ROOT / "scripts" / "instantiate_jarvis.py"
INPUT_FIXTURE = REPO_ROOT / "tests" / "fixtures" / "acme-jarvis-render-input.json"

SPEC = importlib.util.spec_from_file_location("verify_jarvis_output", VERIFIER_SCRIPT)
MODULE = importlib.util.module_from_spec(SPEC)
sys.modules["verify_jarvis_output"] = MODULE
...
class VerifierContractTests(unittest.TestCase):
    ...
    def test_generated_base_satisfies_structural_contract(self) -> None:
        report = self.report(expected_jarvis_slug="acme-claude-e2e")
    ...
    def test_missing_home_and_entry_fail_closed(self) -> None:
        ...
        self.assert_finding(
            self.report(expected_jarvis_slug="acme-claude-e2e"),
    ...
    def test_filesystem_safety_checks_generated_surfaces(self) -> None:
        ...
        unresolved = self.home / "skills" / "__JARVIS_SLUG__-jarvis"
        unresolved.mkdir()
        report = self.report()
        for code in (
            "symlink_not_allowed",
