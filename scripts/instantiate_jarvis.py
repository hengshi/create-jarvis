#!/usr/bin/env python3
"""Deterministic instantiator for Jarvis templates.

Subcommands:
    base    --input <jarvis render input JSON>
...
The base command installs only Jarvis-owned entry/workflow skills and the
...
JARVIS_REPO_TEMPLATE = TEMPLATES_ROOT / "jarvis" / "repo"
JARVIS_MODULE_TEMPLATE = TEMPLATES_ROOT / "jarvis" / "module"
JARVIS_SOURCE_TEMPLATE = TEMPLATES_ROOT / "jarvis" / "source"
...
def jarvis_workflow_name(jarvis_slug: str, workflow_name: str) -> str:
    """Return the canonical Jarvis-owned workflow skill name."""
    prefix = f"{jarvis_slug}-workflow-"
...
def extract_globals(input_data: dict) -> dict:
    ...
    jarvis = input_data.get("jarvis")
    if not isinstance(jarvis, dict):
        print("ERROR: jarvis is missing or not an object", file=sys.stderr)
        sys.exit(1)

    jarvis_name = require_string(
        jarvis.get("name", ""),
        "JARVIS_NAME — jarvis.name",
    )
    jarvis_slug = require_string(
        jarvis.get("slug", ""),
        "JARVIS_SLUG — jarvis.slug",
    )
    jarvis_slug = validate_name(jarvis_slug, "jarvis slug")

    purpose_value = jarvis.get("purpose", "")
    if not isinstance(purpose_value, str):
        print(
            "ERROR: JARVIS_PURPOSE — jarvis.purpose must be a string",
            file=sys.stderr,
        )
        sys.exit(1)
    purpose = purpose_value.strip() or (
    ...
    # JARVIS_OWNER: accept string or list; missing -> UNRESOLVED
    jarvis_owner = jarvis.get("owner", "")
    if isinstance(jarvis_owner, list):
        jarvis_owner = ", ".join(jarvis_owner)
    if not isinstance(jarvis_owner, str) or not jarvis_owner.strip():
        jarvis_owner = "UNRESOLVED"

    starter_workflows = [
        jarvis_workflow_name(jarvis_slug, suffix)
    ...
    globals_ = {
        ...
        "JARVIS_SLUG": jarvis_slug,
        "JARVIS_REPO_NAME": f"{jarvis_slug}-jarvis",
        "JARVIS_PURPOSE": purpose,
        "JARVIS_OWNER": jarvis_owner,
...
def refresh_readme_scope_indexes(
    ...
    *,
    ...
    must still keep the durable Jarvis entry synchronized without overwriting
    ...
    for heading, next_heading, directory, item_format in sections:
        ...
        if heading == "工作流":
            prefix = f"{jarvis_slug}-workflow-"
            names = [name for name in names if jarvis_slug and name.startswith(prefix)]
...
def copy_skill_package(
    ...
    *,
    ...
    """Render one known skill package into the Jarvis repo."""
    package_globals = dict(globals_)
    package_globals["SKILL_NAME"] = name
    if kind == "generic-source":
        jarvis_prefix = f"{globals_['JARVIS_SLUG']}-"
        package_globals["SOURCE_NAME"] = name.removeprefix(jarvis_prefix)
...
def copy_and_render(src_dir: Path, dst_dir: Path, globals_: dict) -> dict:
    """Copy tree, render tokens, rename __JARVIS_SLUG__ dirs."""
    ...
    for src_path in sorted(src_dir.rglob("*")):
        ...
        # Apply jarvis slug rename
        parts = list(rel_path.parts)
        new_parts = []
        for p in parts:
            if "__JARVIS_SLUG__" in p:
                slug = globals_.get("JARVIS_SLUG", "")
                if not slug:
                    result["errors"].append(f"cannot rename __JARVIS_SLUG__ in {rel_path}: JARVIS_SLUG is empty")
                    continue
                new_parts.append(p.replace("__JARVIS_SLUG__", slug))
            else:
                new_parts.append(p)
...
def _resolve_agent_target(target_path: str, workspace_root: str) -> tuple[Path, Path]:
    ...
    try:
        ...
        print(
            f"ERROR: jarvis target must be inside the agent workspace: {target} not under {workspace}",
            file=sys.stderr,
        )
        sys.exit(1)
    if relative == Path("."):
        print("ERROR: jarvis target must be a child of the workspace root", file=sys.stderr)
        sys.exit(1)
...
def cmd_base(input_data: dict) -> int:
    """Instantiate Jarvis repo from templates/jarvis/repo/."""
    globals_ = extract_globals(input_data)
    target, _ = _resolve_input_target(input_data)

    if not globals_.get("JARVIS_SLUG"):
        print("ERROR: JARVIS_SLUG is empty — cannot render jarvis templates", file=sys.stderr)
        return 1

    print(f"base: target={target}, slug={globals_['JARVIS_SLUG']}")
    result = copy_and_render(JARVIS_REPO_TEMPLATE, target, globals_)
    for kind, suffix in STARTER_WORKFLOW_PACKAGES:
        name = jarvis_workflow_name(globals_["JARVIS_SLUG"], suffix)
        merge_copy_results(
            result,
    ...
    refresh_readme_scope_indexes(
        ...
        jarvis_slug=globals_["JARVIS_SLUG"],
    )
...
def cmd_module(input_data: dict, name: str) -> int:
    """Instantiate a single module contract from templates/jarvis/module/."""
    globals_ = extract_globals(input_data)
    target, _ = _resolve_input_target(input_data)
    ...
    if not JARVIS_MODULE_TEMPLATE.is_dir():
        print(f"ERROR: module template directory not found: {JARVIS_MODULE_TEMPLATE}", file=sys.stderr)
        return 1

    result = copy_and_render(JARVIS_MODULE_TEMPLATE, module_dir, globals_)
...
def cmd_source(input_data: dict, name: str) -> int:
    """Instantiate a source route entry from templates/jarvis/source/ (README.md only, no SKILL.md)."""
    globals_ = extract_globals(input_data)
    target, _ = _resolve_input_target(input_data)

    name = validate_name(name, "source name")
    src_template = JARVIS_SOURCE_TEMPLATE
...
def cmd_package(input_data: dict, kind: str, name: str) -> int:
    ...
    if kind == "generic-workflow":
        expected_prefix = f"{jarvis_slug}-workflow-"
    else:
        expected_prefix = f"{jarvis_slug}-"
    ...
    if kind == "generic-source" and (
        name == f"{jarvis_slug}-jarvis"
        or name.startswith(f"{jarvis_slug}-workflow-")
    ):
        print(
            f"ERROR: generic-source output name collides with a reserved jarvis skill: {name!r}",
            file=sys.stderr,
        )
        return 1
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Deterministic Jarvis template instantiator."
    )
    sub = parser.add_subparsers(dest="command", required=True)

    p_base = sub.add_parser("base", help="Instantiate Jarvis repo base")
    p_base.add_argument("--input", required=True, help="Path to jarvis render input JSON")
